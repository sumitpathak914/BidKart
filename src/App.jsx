...
function LayoutWithBottomNav() {
  ...
  // Show BottomNav on all pages EXCEPT hidden paths and when modal is open
  const showBottomNav = !shouldHideBottomNav && !isModalOpen;
  ...
  return (
    <div className="min-h-screen bg-[#F6F5F1]">
      <Routes>
        ...
        <Route
          path="/community"
          element={<CommunityPage onModalOpenChange={setIsModalOpen} />}
        />
        <Route path="/profile" element={<ProfilePage />} />
        <Route path="/scan-qr" element={<QRScannerPage />} />
        <Route path="/shop/:shopId" element={<ShopDetailsPage />} />
        ...
        />
        <Route
          path="/business-profile/:shopId"
          element={<BusinessProfilePage />}
        />
      </Routes>

      {/* BottomNav - Show on all pages EXCEPT "/" and "/my-community" and when modal is open */}
      {showBottomNav && <BottomNav />}
    </div>
  );
}
